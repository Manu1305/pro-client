...
function ExtraData ({Allproductlength,productrequestlength,handleLoginTypeChange}) {
...
const [userLength,setUsersLength]=useState('')
  ...
  // if (orders && orders.ordrPrc && Array.isArray(orders.ordrPrc)) {
  //   var sum = orders.ordrPrc.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
  //   console.log(sum+"sum is ");
  //   setTotalSales(sum)
  // } else {
  //   console.log('orders or ordrPrc is undefined or not an array.');
  // }                             
        ...
        <div className="row g-4" >
          {data.map((item, index) => (
            <div className="col-sm-6 col-xl-3" key={index}   >
              <div className="bg-light rounded d-flex align-items-center justify-content-between p-4" onClick={()=>{
                handleLoginTypeChange(item.link)
              }} style={{backgroundColor:'#D9D3D3'}} >
                <i className={item.icon}></i>
                <div className="ms-3">
                  <p className="mb-2" style={{fontWeight:'bolder',color:'black'}} >{item.label}</p>
